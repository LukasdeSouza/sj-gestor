...
function SectionTitle({ children }: { children: React.ReactNode }) {
  return (
    <h2 style={{ fontFamily: "'Montserrat', sans-serif", fontSize: "1.05rem", fontWeight: 800, color: "#0F172A", letterSpacing: -0.3, margin: "0 0 0.85rem" }}>
      {children}
    </h2>
  );
}
...
function SubTitle({ children }: { children: React.ReactNode }) {
  return (
    <h3 style={{ fontFamily: "'Montserrat', sans-serif", fontSize: "0.85rem", fontWeight: 700, color: "#334155", margin: "1.1rem 0 0.35rem" }}>
      {children}
    </h3>
  );
}
...
function Prose({ children }: { children: React.ReactNode }) {
  return (
    <p style={{ fontSize: "0.82rem", color: "#64748B", lineHeight: 1.75, margin: 0 }}>
      {children}
    </p>
  );
}
...
function InfoBox({ children, type = "info" }: { children: React.ReactNode; type?: "info" | "warn" | "tip" }) {
  ...
  return (
    <div style={{ background: s.bg, border: `1px solid ${s.border}`, borderRadius: 9, padding: "0.75rem 1rem", display: "flex", gap: 8, margin: "0.75rem 0" }}>
      ...
      <span style={{ fontSize: "0.8rem", color: "#64748B", lineHeight: 1.65 }}>{children}</span>
    </div>
  );
}
...
function Step({ n, title, children }: { n: number; title: string; children?: React.ReactNode }) {
  return (
    <div style={{ display: "flex", gap: 12, padding: "0.75rem 0", borderBottom: "1px solid #F1F5F9" }}>
      <div style={{
        ...
        fontFamily: "'Montserrat', sans-serif", flexShrink: 0, marginTop: 1,
      }}>
        {n}
      </div>
      <div>
        <div style={{ fontSize: "0.82rem", fontWeight: 600, color: "#0F172A", marginBottom: 2 }}>{title}</div>
        {children && <div style={{ fontSize: "0.78rem", color: "#64748B", lineHeight: 1.6 }}>{children}</div>}
      </div>
    </div>
  );
}
...
function VarPill({ tag, label }: { tag: string; label: string }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "0.45rem 0.85rem", background: "#F8FAFC", border: "1px solid #E2E8F0", borderRadius: 8 }}>
...
function FaqItem({ q, a }: { q: string; a: string }) {
  ...
  return (
    <div style={{ borderBottom: "1px solid #F1F5F9" }}>
      ...
      >
        <span style={{ fontSize: "0.83rem", fontWeight: 600, color: "#0F172A", fontFamily: "'Montserrat', sans-serif" }}>{q}</span>
        <ChevronRight size={14} style={{ color: "#64748B", flexShrink: 0, transform: open ? "rotate(90deg)" : "none", transition: "transform 0.2s" }} />
      </button>
      {open && (
  ...
  return (
    <div style={{ background: "rgba(0,200,150,0.06)", border: "1px solid rgba(0,200,150,0.15)", borderRadius: 10, padding: "0.85rem 1rem", display: "flex", alignItems: "center", gap: 10 }}>
      ...
      <div>
        ...
        <div style={{ fontSize: "0.76rem", color: "#64748B" }}>Segunda a Sexta: 9h às 18h &nbsp;·&nbsp; Sábado: 9h às 12h</div>
      </div>
    </div>
  );
}
  ...
  <div style={{ display: "flex", flexDirection: "column", gap: 0 }}>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.65rem", margin: "1rem 0" }}>
      {[
        ...
        <div key={i} style={{ background: "#FFFFFF", border: "1px solid #E2E8F0", borderRadius: 10, padding: "0.85rem 1rem", display: "flex", gap: 10 }}>
          ...
          <div>
            <div style={{ fontSize: "0.8rem", fontWeight: 700, color: "#0F172A", marginBottom: 2 }}>{f.title}</div>
            <div style={{ fontSize: "0.74rem", color: "#64748B", lineHeight: 1.5 }}>{f.desc}</div>
          </div>
        </div>
      ))}
    </div>
...
const ClientsContent = () => (
  <div>
    ...
    <Prose>Na lista de clientes, clique em "detalhes" sobre o cliente desejado. Dentro do modal, clique em "Registrar Pagamento" e informe o valor e a data.</Prose>
  ...
  <div>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: "0.5rem", marginTop: "0.5rem" }}>
      {[
        ...
        <div key={h.period} style={{ background: "#F8FAFC", border: "1px solid #E2E8F0", borderRadius: 9, padding: "0.6rem 0.85rem", textAlign: "center" }}>
          <div style={{ fontSize: "0.72rem", color: "#64748B", fontWeight: 700, textTransform: "uppercase", letterSpacing: 0.5 }}>{h.period}</div>
          <div style={{ fontFamily: "'Montserrat', sans-serif", fontSize: "0.88rem", fontWeight: 800, color: "#00C896", marginTop: 2 }}>{h.time}</div>
        </div>
      ))}
    </div>
...
const WhatsAppIssuesContent = () => (
  <div>
    ...
    {[
      ...
      <div key={i} style={{ display: "flex", gap: 10, padding: "0.85rem 0", borderBottom: "1px solid #F1F5F9" }}>
        <XCircle size={14} style={{ color: "#E84545", flexShrink: 0, marginTop: 3 }} />
        <div>
          <div style={{ fontSize: "0.82rem", fontWeight: 700, color: "#0F172A", marginBottom: 4, fontFamily: "'Montserrat', sans-serif" }}>{item.q}</div>
          <div style={{ fontSize: "0.78rem", color: "#64748B", lineHeight: 1.65 }}>{item.a}</div>
        </div>
      </div>
    ))}
...
export default function Help() {
  ...
  const sections: Section[] = [
    ...
    { id: "whatsapp-issues", label: "Problemas", icon: AlertTriangle, accent: "#E84545", accentBg: "rgba(232,69,69,0.08)", content: <WhatsAppIssuesContent /> },
  ...
  return (
    <>
      ...
      <DashboardLayout>
        <div style={{ fontFamily: " 'Montserrat', sans-serif", color: "#0F172A", display: "flex", flexDirection: "column", gap: "1.25rem" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <div>
              <h1 style={{ fontFamily: "'Montserrat', sans-serif", fontSize: "1.3rem", fontWeight: 800, color: "#0F172A", letterSpacing: -0.5, margin: 0 }}>
                Central de Ajuda
              </h1>
              <p style={{ fontSize: "0.78rem", color: "#64748B", margin: 0 }}>Tutoriais, guias e suporte</p>
            </div>
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "200px 1fr", gap: "1rem", alignItems: "start" }}>
            ...
            {/* SIDEBAR NAV */}
            <div style={{ background: "#FFFFFF", border: "1px solid #E2E8F0", borderRadius: 12, padding: "0.5rem", display: "flex", flexDirection: "column", gap: 2, position: "sticky", top: "1rem" }}>
              {sections.map((s) => {
                ...
                return (
                  <button
                    ...
                    style={{
                      ...
                      color: isActive ? "#00C896" : (s.accent ?? "#64748B"),
                      fontSize: "0.78rem", fontWeight: isActive ? 700 : 500,
                      fontFamily: isActive ? "'Montserrat', sans-serif" : " 'Montserrat', sans-serif",
