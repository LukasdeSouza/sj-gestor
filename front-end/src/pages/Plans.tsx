...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span style={{ display: "inline-flex", alignItems: "center", gap: 5, background: s.bg, color: s.color, border: `1px solid ${s.border}`, borderRadius: 100, padding: "3px 10px", fontSize: 11, fontWeight: 700, fontFamily: "'Montserrat', sans-serif" }}>
      {s.label}
    </span>
  );
}
...
function PlanIcon({ id }: { id: PlanId }) {
  if (id === "FREE") return <Users size={18} color="var(--text2)" />;
  ...
  return <ShieldCheck size={18} color="var(--text2)" />;
}

function planAccent(id: PlanId) {
  if (id === "FREE") return { color: "var(--text2)", border: "rgba(90,122,112,0.2)", bg: "rgba(90,122,112,0.06)" };
  ...
  return { color: "var(--text2)", border: "var(--border)", bg: "transparent" };
}
...
export default function Plans() {
  ...
  const tabBtn = (id: "plans" | "history"): React.CSSProperties => ({
    ...
    color: activeTab === id ? "var(--cobr)" : "var(--text2)",
  ...
  return (
    <>
      <style>{`
        ...
        .pl-card:hover { border-color: rgba(0,200,150,0.3) !important; }
        .pl-btn-primary { display:inline-flex;align-items:center;justify-content:center;gap:6px;background:var(--cobr);color:#fff;border:none;border-radius:8px;padding:0.65rem 1.1rem;font-family:'Montserrat',sans-serif;font-size:0.82rem;font-weight:700;cursor:pointer;transition:background 0.2s;width:100%; text-decoration: none; }
        .pl-btn-primary:hover:not(:disabled){background:var(--cobr-hover);}
        .pl-btn-primary:disabled{opacity:0.55;cursor:not-allowed;}
        .pl-btn-ghost{display:inline-flex;align-items:center;justify-content:center;gap:6px;background:var(--bg);border:1px solid var(--border);color:var(--text2);border-radius:8px;padding:0.65rem 1.1rem;font-family:'Montserrat',sans-serif;font-size:0.82rem;font-weight:700;cursor:pointer;transition:all 0.15s;width:100%;}
        .pl-btn-ghost:hover:not(:disabled){border-color:rgba(0,200,150,0.25);color:var(--text1);}
        .pl-btn-ghost:disabled{opacity:0.4;cursor:not-allowed;}
        .pl-btn-outline{display:inline-flex;align-items:center;justify-content:center;gap:6px;background:transparent;border:1px solid var(--border);color:var(--text2);border-radius:8px;padding:0.65rem 1.1rem;font-family:'Montserrat',sans-serif;font-size:0.82rem;font-weight:700;cursor:pointer;transition:all 0.15s;width:100%;}
        .pl-btn-outline:hover:not(:disabled){border-color:var(--cobr);color:var(--cobr);}
        .pl-tab:hover{color:var(--text1) !important;}
        .pl-proof-btn:hover{border-color:rgba(0,200,150,0.3) !important;color:var(--cobr) !important;}
        .pl-modal-close:hover{color:var(--text1) !important;}
        ...
        .welcome-badge {
          ...
          background: rgba(0,200,150,0.1); color: var(--cobr);
        ...
        .welcome-title {
          font-family: 'Montserrat', sans-serif; font-size: 2rem; fontWeight: 800;
          color: var(--text1); line-height: 1.1; letter-spacing: -1.2px;
          margin-bottom: 1rem;
        }
        .welcome-subtitle {
          color: var(--text2); font-size: 1rem; line-height: 1.6;
        ...
        .welcome-item-title {
          font-family: 'Montserrat', sans-serif; color: var(--text1);
          font-size: 0.95rem; fontWeight: 700; margin-bottom: 4px;
        }
        .welcome-item-desc { color: var(--text2); font-size: 0.85rem; line-height: 1.5; font-weight: 500 }
        .welcome-cta {
          width: 100%; background: var(--cobr); color: #fff;
          border: none; border-radius: 12px; padding: 1.25rem;
          font-family: 'Montserrat', sans-serif; font-size: 1rem; fontWeight: 800;
          cursor: pointer; display: flex; align-items: center; justify-content: center;
          gap: 10px; transition: all 0.2s;
        }
        .welcome-cta:hover { background: var(--cobr-hover); transform: translateY(-2px); }
      ...
      <DashboardLayout>
        <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem", fontFamily: " 'Montserrat', sans-serif", color: "var(--text1)" }}>

          {/* ── HEADER ── */}
          <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 16, flexWrap: "wrap" }}>
            <div>
              <h1 style={{ fontFamily: "'Montserrat', sans-serif", fontSize: "1.3rem", fontWeight: 800, color: "var(--text1)", letterSpacing: -0.5, margin: "0 0 3px" }}>
                Assinatura
              </h1>
              <p style={{ fontSize: "0.8rem", color: "var(--text2)", margin: 0 }}>Gerencie seu plano e histórico de pagamentos</p>
            </div>
            <LogoutButton variant="outline" />
          </div>

          {/* ── TABS ── */}
          <div style={{ display: "flex", gap: 4, background: "var(--bg2)", border: "1px solid var(--border)", borderRadius: 10, padding: 4, width: "fit-content" }}>
              ...
              {plans?.map((p) => {
                ...
                return (
                  <div
                    ...
                    className="pl-card"
                    style={{
                      background: isCurrent ? accent.bg : "var(--bg2)",
                      border: `1px solid ${isCurrent ? accent.border : "var(--border)"}`,
                  ...
                  >
                    ...
                    {p.id === "PRO" && (
                      <div style={{ position: "absolute", top: 0, right: 0 }}>
                        <div style={{ background: "#818CF8", color: "#F0F5F2", fontSize: 9, fontWeight: 800, fontFamily: "'Montserrat', sans-serif", padding: "3px 10px", borderRadius: "0 0 0 8px" }}>
                          RECOMENDADO
                        </div>
                      </div>
                    )}
                    {p.id === "ELITE" && (
                      <div style={{ position: "absolute", top: 0, right: 0 }}>
                        <div style={{ background: "#F5A623", color: "#051A12", fontSize: 9, fontWeight: 800, fontFamily: "'Montserrat', sans-serif", padding: "3px 10px", borderRadius: "0 0 0 8px" }}>
                          MAIS VANTAGEM
                        </div>
                      </div>
                    )}
                    ...
                    <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                        ...
                        <div>
                          <div style={{ fontFamily: "'Montserrat', sans-serif", fontSize: "0.9rem", fontWeight: 800, color: "var(--text1)" }}>{p.name}</div>
                          <div style={{ fontSize: 11, color: "var(--text2)" }}>{p.description}</div>
                        </div>
                      </div>
                      {isCurrent && (
                        <span style={{ background: isActive ? "rgba(0,200,150,0.1)" : "rgba(245,166,35,0.1)", color: isActive ? "#00C896" : "#F5A623", border: `1px solid ${isActive ? "rgba(0,200,150,0.2)" : "rgba(245,166,35,0.2)"}`, borderRadius: 100, padding: "2px 8px", fontSize: 10, fontWeight: 700, fontFamily: "'Montserrat', sans-serif", whiteSpace: "nowrap" }}>
                    ...
                    {/* Price */}
                    <div>
                      <div style={{ fontFamily: "'Montserrat', sans-serif", fontSize: "1.8rem", fontWeight: 800, color: accent.color, letterSpacing: -1.5, lineHeight: 1 }}>
                        {p.price === 0 ? "Grátis" : fmtCurrency(p.price)}
                      </div>
                      {p.price > 0 && <div style={{ fontSize: 11, color: "var(--text2)", marginTop: 3 }}>/mês</div>}

                      {/* Features list */}
                      <div style={{ marginTop: 12, display: "flex", flexDirection: "column", gap: 6 }}>
                        <div style={{ fontSize: 11, color: "var(--text2)", display: "flex", alignItems: "center", gap: 6 }}>
                          <Users size={12} color="#00C896" />
                    ...
                    {isCurrent && isActive && expiresAt && (
                      <div style={{ background: "var(--bg)", border: "1px solid var(--border)", borderRadius: 9, padding: "0.65rem 0.85rem" }}>
                        <div style={{ fontSize: 10, color: "var(--text2)", fontWeight: 700, textTransform: "uppercase", letterSpacing: 0.8, marginBottom: 3 }}>Vencimento</div>
                        <div style={{ fontSize: 12, fontWeight: 600, color: "var(--text1)" }}>{fmtDateLong(expiresAt)}</div>
                      </div>
                    )}
          ...
          {activeTab === "history" && (
            !userPayment ? (
              <div style={{ background: "var(--bg2)", border: "1px solid var(--border)", borderRadius: 14, padding: "3rem", textAlign: "center", color: "var(--text2)" }}>
              ...
              <div style={{ background: "var(--bg2)", border: "1px solid var(--border)", borderRadius: 14, overflow: "hidden" }}>
                {/* Payment header */}
                <div style={{ padding: "1.25rem 1.5rem", borderBottom: "1px solid var(--border)", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
                  <div>
                    <div style={{ fontFamily: "'Montserrat', sans-serif", fontSize: "0.9rem", fontWeight: 800, color: "var(--text1)" }}>
                ...
                <div style={{ padding: "1.25rem 1.5rem" }}>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.85rem", marginBottom: "1.25rem" }}>
                    <div style={{ background: "var(--bg)", border: "1px solid var(--border)", borderRadius: 9, padding: "0.75rem 1rem" }}>
                      <div style={{ fontSize: 10, color: "var(--text2)", fontWeight: 700, textTransform: "uppercase", letterSpacing: 0.8, marginBottom: 4 }}>Plano</div>
                      <div style={{ fontSize: 12, fontWeight: 600, color: "var(--text1)" }}>{userPayment.planId}</div>
                    </div>
                    <div style={{ background: "var(--bg)", border: "1px solid var(--border)", borderRadius: 9, padding: "0.75rem 1rem" }}>
                      <div style={{ fontSize: 10, color: "var(--text2)", fontWeight: 700, textTransform: "uppercase", letterSpacing: 0.8, marginBottom: 4 }}>Valor</div>
                      <div style={{ fontFamily: "'Montserrat', sans-serif", fontSize: "1rem", fontWeight: 800, color: "#00C896" }}>
                  ...
                  <div style={{ marginTop: "0.5rem" }}>
                    <div style={{ fontSize: 10, color: "var(--text2)", fontWeight: 700, textTransform: "uppercase", letterSpacing: 0.8, marginBottom: "0.85rem" }}>Histórico</div>
                      ...
                      {userPayment.proofUploadedAt && (
                        <div style={{ display: "flex", gap: 12 }}>
                          <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
                            ...
                            {(userPayment.approvedAt || userPayment.rejectedAt || userPayment.canceledAt) && <div style={{ width: 1, flex: 1, background: "var(--border)", marginTop: 4, minHeight: 28 }} />}
                          </div>
                          <div style={{ paddingBottom: 16 }}>
                            <div style={{ fontSize: 13, fontWeight: 600, color: "var(--text1)" }}>Comprovante enviado</div>
                            <div style={{ fontSize: 11, color: "var(--text2)", marginTop: 2 }}>{fmtDate(userPayment.proofUploadedAt)}</div>
                            {userPayment.proofUrl && (
                              <button
                                className="pl-proof-btn"
                                onClick={() => setSelectedProof(userPayment.proofUrl)}
                                style={{ marginTop: 6, background: "none", border: "1px solid var(--border)", color: "var(--text2)", borderRadius: 7, padding: "4px 10px", fontSize: 11, fontWeight: 600, cursor: "pointer", display: "inline-flex", alignItems: "center", gap: 5, transition: "all 0.15s" }}
                              >
        ...
        {showProofForm && pixPaymentData && (
          <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 50 }}>
            <div style={{ background: "var(--bg2)", border: "1px solid var(--border)", borderRadius: 16, padding: "1.5rem", maxWidth: 420, width: "100%", margin: "1rem" }}>
              <div style={{ fontFamily: "'Montserrat', sans-serif", fontSize: "1rem", fontWeight: 800, color: "var(--text1)", marginBottom: "1rem" }}>
                Enviar Comprovante
              </div>
              <ProofUploadForm
        ...
        {selectedProof && (
          <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 50 }}>
            <div style={{ background: "var(--bg2)", border: "1px solid var(--border)", borderRadius: 16, padding: "1.5rem", maxWidth: 560, width: "100%", margin: "1rem" }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "1rem" }}>
                <div style={{ fontFamily: "'Montserrat', sans-serif", fontSize: "0.95rem", fontWeight: 800, color: "var(--text1)" }}>
                  Comprovante de Pagamento
                </div>
                <button
                  className="pl-modal-close"
                  onClick={() => setSelectedProof(null)}
                  style={{ background: "none", border: "none", cursor: "pointer", color: "var(--text2)", fontSize: 18, lineHeight: 1, transition: "color 0.15s" }}
                >
                  ✕
                </button>
              </div>
              {selectedProof.endsWith(".pdf") ? (
                <div style={{ background: "var(--bg)", border: "1px solid var(--border)", borderRadius: 10, padding: "1.5rem", textAlign: "center" }}>
                  <p style={{ fontSize: 12, color: "var(--text2)", marginBottom: "1rem" }}>Arquivo PDF</p>
